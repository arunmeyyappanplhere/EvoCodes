// serviceColor can be a hex string (e.g. "#22d3ee") or a Tailwind class string
// (e.g. "bg-cyan-500/15 text-cyan-300").
...
// ── Tailwind v3 color palette (color name → hex) ──────────────────────────
const TW = {
  ...
  'zinc-50':'#fafafa','zinc-100':'#f4f4f5','zinc-200':'#e4e4e8','zinc-300':'#d4d4d8',
...
// Default fallback hex (cyan-400)
const FALLBACK = '#22d3ee'

// ── Helpers ───────────────────────────────────────────────────────────────

/** Parse a single Tailwind color token like "cyan-500/15" → { r, g, b, a } or null */
function parseTwToken(token) {
  ...
  return {
    ...
    a: m[2] ? parseInt(m[2]) / 100 : 1,
    hex,
  }
}

/**
 * Parse a service colour value into a normalised object.
 *
 * Accepts:
 *   - Hex string:  "#06b6d4"
 *   - Tailwind:    "bg-cyan-500/15 text-cyan-300"
 *   - nothing:     null / undefined / empty
 *
 * Returns { type, bg?, text? } where bg/text are { r, g, b, a, hex }.
 */
export function parseServiceColor(color) {
  ...
  let bg = null; let text = null
  parts.forEach((p) => {
    if (p.startsWith('bg-'))   bg   = parseTwToken(p.slice(3))
    else if (p.startsWith('text-')) text = parseTwToken(p.slice(5))
  })
  return bg || text ? { type: 'tailwind', bg, text } : null
}
 ...
 * Falls back to cyan-400 if the input is not a valid hex.
 */
...
/**
 * Returns an inline style object for a "chip" (icon box, badge pill).
 *
 * Handles both hex colours and Tailwind class strings like
 * "bg-cyan-500/15 text-cyan-300".
 */
export const chipStyle = (color) => {
  const parsed = parseServiceColor(color)
  if (!parsed) return { color: FALLBACK }

  if (parsed.type === 'hex' && parsed.bg) {
    const { r, g, b } = parsed.bg
    return {
  ...
  if (parsed.type === 'tailwind') {
    const style = {}
    if (parsed.bg) {
      const { r, g, b, a } = parsed.bg
      style.backgroundColor = `rgba(${r}, ${g}, ${b}, ${a})`
      style.borderColor     = `rgba(${r}, ${g}, ${b}, ${Math.min(a + 0.15, 1)})`
    }
    if (parsed.text) {
      const { r, g, b } = parsed.text
...
/**
 * Returns an inline style object for themed text (titles, labels).
 *
 * Handles both hex colours and Tailwind class strings.
 */
export const textStyle = (color) => {
  ...
  if (parsed.type === 'tailwind' && parsed.text) {
    const { r, g, b } = parsed.text
